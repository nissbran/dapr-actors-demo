...
public static class CheckoutApi
{
    public static void MapCheckoutApi(this WebApplication app)
    {
        ...
        group.MapPost("{id}/re-entrance", ReEntrance);
    ...
    private static async Task<IResult> ReEntrance(string id, [FromServices] IActorProxyFactory factory)
    {
        var order = factory.GetCheckoutActor(id);
        await order.CompleteAndSendOrder();
        return TypedResults.Ok();
    }
    
    private static ICheckoutOrder GetCheckoutActor(this IActorProxyFactory factory, string id)
    {
